...
可以看出，活动的会话稳定在13个，离最大个数20已经不远，极有可能是数据库会话配置过小，导致用户增多时，无可用连接分配。为了验证推断，使用JMeter模拟多用户同时访问测试网站。逐步增加同时访问的用户的个数，当增加到80个时，网站无响应。检查后端日志，出现了线上的问题。DBCP当连接数超过最大连接数时，所有连接都会被断开。
...
做了以上优化后，可以提高网站的并发服务量，20个连接确实太小，综评估资源，调整后连接池分配情况：
