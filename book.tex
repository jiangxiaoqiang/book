...
由于目前应用较多，单独列出每个应用的目录比较冗长。此处仅仅描述目录的命名的一般性原则。当前各个应用的目录如下：
...
\begin{table}[htbp]
	...
	\begin{center}
		\begin{tabular}{|c|c|p{5cm}|}
			...
			\multirow{1}{*}{项目名称}
			& \multicolumn{1}{c|}{环境} 
			& \multicolumn{1}{c|}{部署目录}\\			
			\cline{1-3}
			report &  Test(6)  & /opt/app/backend-v1.3/report \\
			\hline
			report &  Production(6)  & /opt/app/backend-v1.3/report \\
			\hline
			report-frontend & Test & /opt/app/frontend-v1.3 \\
			\hline
			report-frontend & Production & /opt/app/frontend-v1.3 \\
			\hline
			system & Test &  /opt/app/frontend-v1.3 \\
			\hline							
		\end{tabular}	
	\end{center}
\end{table}
