...
将localhost调整为IP后，服务端消费时提示Broker may not be available，是由于为了支持外部访问Kafka，server.propertise文件中指定绑定IP，此时生产消费脚本也需要通过IP来进行。advertised.listeners为Kafka 0.9.x以后的版本新增配置，原来的advertise.host.name和port配置不建议再使用。使用命令创建Topic:
...
\begin{lstlisting}[language=Bash]
# 消费dolphin-test主题消息
bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic dolphin-test --from-beginning
# 生产dolphin-test主题消息
bin/kafka-console-producer.sh --broker-list localhost:9092 --topic dolphin-test
\end{lstlisting}
