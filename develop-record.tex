...
<<<<<<< .mine\subsection{/tmp/spring.log (Permission denied)}
=======\subsection{配置(Configuration)}
>>>>>>> .theirs\subsection{/tmp/spring.log (Permission denied)}
...
\end{lstlisting}
<<<<<<< .mine启动应用程序时，提示权限拒绝。是由于在logback.xml配置中引入了spring默认的日志配置，spring将日志写入到默认的文件夹tmp下，而普通用户没有权限写入文件内容到此文件夹，导致此问题。去掉默认的引入即可：
...
一旦测试并且验证了你的公钥可以登录，就可以取消密码登录，这样你的远程服务器就不会被暴力密码攻击。
